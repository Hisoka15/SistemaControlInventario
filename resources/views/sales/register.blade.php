...
@section('HomeTemplateContent')
    <form class="text-xs flex flex-col gap-2 w-full max-w-[60rem] mx-auto" action="{{route('sales.store')}}">
        <h1 class="text-2xl font-bold py-4">Registro de venta</h1>
        @csrf
        <div class="flex w-full grid grid-cols-12 [&>input]:col-span-4 [&>label]:col-span-2 justify-center items-center gap-2">
            <label>Cliente:</label>
            <input type="text" name="client" class="border p-2 w-full" required>
            <label>Total:</label>
        ...
        <div class="flex justify-end">
            <x-modal buttonText="AGREGAR PRODUCTO">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <button type="button" id="sale_btn" class="w-full bg-blue-500 text-white px-4 py-2 rounded-md mt-2">Agregar producto</button>
                </div>
            </x-modal>
            <input type="hidden" name="saleDetails" id="saleDetailsInput">
        </div>
        ...
        <button type="button" id="btnSendPurchase" class="w-full bg-green-500 text-white px-4 py-2 rounded-md mt-2">Registrar venta</button>
    </form>
